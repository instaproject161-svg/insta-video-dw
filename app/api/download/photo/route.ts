...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("Downloading photo for URL:", url);
    const result = await fetchInstagramMedia(url.trim(), "photo");
    return NextResponse.json({ success: true, result: { ...result, type: "photo" } });
  } catch (err) {
