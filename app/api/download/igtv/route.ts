...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("Downloading IGTV for URL:", url);
    const result = await fetchInstagramMedia(url.trim(), "igtv");
    return NextResponse.json({ success: true, result: { ...result, type: "igtv" } });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Internal server error";
    console.error("IGTV download error:", message);
