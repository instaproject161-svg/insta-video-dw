...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("Downloading carousel for URL:", url);
    const result = await fetchInstagramMedia(url.trim(), "carousel");
    return NextResponse.json({ success: true, result: { ...result, type: "carousel" } });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Internal server error";
    console.error("Carousel download error:", message);
