...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("Downloading profile for URL:", url);
    const result = await fetchInstagramMedia(url.trim(), "profile");
    return NextResponse.json({ success: true, result: { ...result, type: "profile" } });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Internal server error";
    console.error("Profile download error:", message);
